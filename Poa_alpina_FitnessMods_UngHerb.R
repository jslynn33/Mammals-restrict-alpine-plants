### Q2 Biomass, flowering, and survival for Poa alpina
...
padat$allolivebio <- 0.420126+ 0.0097468*padat$tiller+0.001770*padat$tiller^2

### biomass models for Poa alpina

biodat <- padat[!is.na(padat$allolivebio),]
...
flower.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:148){en[j]~dnorm(0, prec2)} # individual
  for (i in 1:N){
...
pasurv <- datsurv[datsurv$species=="POAL",]
head(pasurv)
...
# independent variables
survmat <- model.matrix(~site*cage*gopher, pasurv)
