### Q2 Biomass, flowering, and survival for Festuca brachyphylla
...
fbdat$allolivebio <- 2.087e-02*fbdat$tiller+1.444e-04*fbdat$tiller^2
  


### biomass models for Festuca brachyphylla
biodat <- fbdat[!is.na(fbdat$allolivebio),]
...
biomass.mod <- function(){
  ...
  for (j in 1:172){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
...
# model checks
s <- ggs(as.mcmc(biojags))
ggs_traceplot(s, family="a") # good mixing

gelman.diag(as.mcmc(biojags))
...
flower.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:133){en[j]~dnorm(0, prec2)} # indiviudal
  for (i in 1:N){
...
fbsurv <- datsurv[datsurv$species=="FEBR",]
head(fbsurv)
...
# independent variables
survmat <- model.matrix(~site*cage*gopher, fbsurv)
