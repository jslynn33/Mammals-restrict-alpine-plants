### Q1 Analysis of damage on Elymus scribneri (ES)
...
presdat <- esdat[!is.na(esdat$presherb),]
...
dampres.mod <- function(){
  ...
  for (j in 1:180){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
    pres[i]~dbern(mu[i])
...
damall.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:151){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
...
damins.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:132){en[j]~dnorm(0, prec2)} # individual random effects
  for (i in 1:N){
    dam[i]~ dnorm(mu[i], prec3) 
...
dammam.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:47){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
    dam[i]~ dnorm(mu[i], prec3) 
