### Q1 Analysis of damage on Poa alpina
...
#data for each species-- here, POAL or Poa alpina
padat <- dat[dat$species=="POAL",]


# Begin model of herbivory presence
presdat <- padat[!is.na(padat$presherb),]
...
dampres.mod <- function(){
  ...
  for (j in 1:178){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
    pres[i]~dbern(mu[i])
...
damall.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:119){en[j]~dnorm(0, prec2)} # individual random effect
  for (i in 1:N){
...
damins.mod <- function(){
  for (j in 1:9){d[j]~dnorm(0, prec1)}# site random effects
  for (j in 1:109){en[j]~dnorm(0, prec2)} # individual random effects
  for (i in 1:N){
    dam[i]~ dnorm(mu[i], prec3) 
...
# independent variables
doy <- mamdamdat$doy
mamdammat <- model.matrix(~site*gopher, data=mamdamdat)
head(mamdammat)
...
dammam.mod <- function(){
  ...
  #priors
  for(j in 1:6){a[j]~ dnorm(0,1.0E-6)}
  bt~dnorm(0,1.0E-6)
  prec1~dgamma(0.001,0.001) 
  prec2~dgamma(0.001,0.001) 
  prec3~dgamma(0.001,0.001) 
